...
global num_epochs, batch_size, valid_batch_size, shallow #, dropout
...
from tensorflow.keras.layers import Input, Dense, MaxPool2D, GlobalMaxPool2D, Dropout, MaxPooling2D, GlobalAveragePooling2D
from tensorflow.keras.models import Model, Sequential
from tensorflow.keras.callbacks import ModelCheckpoint, EarlyStopping, ReduceLROnPlateau
from tensorflow.keras.layers import DepthwiseConv2D, Conv2D, BatchNormalization, Activation, concatenate
...
###===================================================
def get_data_generator_1image(df, indices, for_training, ID_MAP, var, batch_size, greyscale):
    """
...
###===================================================
def predict_test_train_siso_simo(train_df, test_df, train_idx, test_idx, vars, SM, weights_path, name, mode, greyscale, CS, dropout): 
    """
    ...
    train_gen = get_data_generator_Nvars_siso_simo(train_df, train_idx, False, vars, len(train_idx), greyscale, CS)
    ...
    del tmp
    tmp = SM.predict(x_train, batch_size=1) #128) 
    if len(vars)>1:
       counter = 0
       for v in vars:
    ...
    test_gen = get_data_generator_Nvars_siso_simo(test_df, test_idx, False, vars, len(test_idx), greyscale, CS)
    ...
    if len(vars)>1:
       counter = 0
       for v in vars:
          exec(v+'_true = np.squeeze(CS[counter].inverse_transform(tmp[counter].reshape(-1,1)))')
          counter +=1
    else:
       exec(vars[0]+'_true = np.squeeze(CS[0].inverse_transform(tmp[0].reshape(-1,1)))')

    for v in vars:
       exec(v+'_P = []')

    del tmp
    tmp = SM.predict(x_test, batch_size=1) #128)
    if len(vars)>1:
       counter = 0
       for v in vars:
          exec(v+'_P.append(np.squeeze(CS[counter].inverse_transform(tmp[counter].reshape(-1,1))))')
          counter +=1
    else:
       exec(vars[0]+'_P.append(np.asarray(np.squeeze(CS[0].inverse_transform(tmp.reshape(-1,1)))))') 

    del tmp, test_gen, x_test
    ...
    for k in range(1,1+(nrows*ncols)):
      try:
         ...
         plt.text(np.nanmin(x), 0.96*np.max(np.hstack((x,y))),'Test : '+str(np.mean(100*(np.abs(eval(vars[k-1]+'_pred') - eval(vars[k-1]+'_true')) / eval(vars[k-1]+'_true'))))[:5]+' %',  fontsize=8, color='b')
         plt.text(np.nanmin(x), np.max(np.hstack((x,y))),'Train : '+str(np.mean(100*(np.abs(eval(vars[k-1]+'_predT') - eval(vars[k-1]+'_trueT')) / eval(vars[k-1]+'_trueT'))))[:5]+' %', fontsize=8)
         plt.title(r''+labs[k-1]+') '+vars[k-1], fontsize=8, loc='left')

         varstring = ''.join([str(k)+'_' for k in vars])
      except:
          pass
    plt.savefig(weights_path.replace('.hdf5', '_skill.png'), dpi=300, bbox_inches='tight')
    plt.close()
    del fig
...
###===================================================
def predict_test_train_miso_mimo(train_df, test_df, train_idx, test_idx, vars, auxin, SM, weights_path, name, mode, greyscale, CS, CSaux): 
    """
    This function creates makes predcitions on test and train data
    """
    ##==============================================
    ...
    train_gen = get_data_generator_Nvars_miso_mimo(train_df, train_idx, False, vars, auxin,aux_mean, aux_std, len(train_idx), greyscale)    
...
#    tmp = SM.predict(x_train, batch_size=64)
#    if len(vars)>1:
#       counter = 0
#       for v in vars:
...
#       exec(vars[0]+'_PT.append(np.asarray(np.squeeze(tmp)))') 
    ...
    ## make predictions on testing data
    test_gen = get_data_generator_Nvars_miso_mimo(test_df, test_idx, False, vars, auxin, aux_mean, aux_std, len(test_idx), greyscale)
...
#    tmp = SM.predict(x_test,batch_size=64)
#    if len(vars)>1:
#       counter = 0
#       for v in vars:
...
#          counter +=1
#    else:
#       exec(vars[0]+'_P.append(np.asarray(np.squeeze(tmp)))') 
...
def  plot_train_history_1var_mae(history):
   ...
   axes[0].plot(np.log(history.history['val_loss']), label='Log Validation loss')
   axes[0].set_xlabel('Epochs')
   axes[0].legend()
...
###===================================================
def get_data_generator_Nvars_miso_mimo(df, indices, for_training, vars, auxin, batch_size, greyscale, CS, CSaux):
    """
    ...
    while True:
        for i in indices:
            ...
            if len(vars)==2:
               file, p1, p2, aa = r['files'], r[vars[0]], r[vars[1]], r[auxin]         
            if len(vars)==3:
               file, p1, p2, p3, aa = r['files'], r[vars[0]], r[vars[1]], r[vars[2]], r[auxin]       
            if len(vars)==4:
               file, p1, p2, p3, p4, aa = r['files'], r[vars[0]], r[vars[1]], r[vars[2]], r[vars[3]], r[auxin]          
            if len(vars)==5:
               file, p1, p2, p3, p4, p5, aa = r['files'], r[vars[0]], r[vars[1]], r[vars[2]], r[vars[3]], r[vars[4]], r[auxin]            
            if len(vars)==6:
               file, p1, p2, p3, p4, p5, p6, aa = r['files'], r[vars[0]], r[vars[1]], r[vars[2]], r[vars[3]], r[vars[4]], r[vars[5]], r[auxin]         
            if len(vars)==7:          
               file, p1, p2, p3, p4, p5, p6, p7, aa = r['files'], r[vars[0]], r[vars[1]], r[vars[2]], r[vars[3]], r[vars[4]], r[vars[5]], r[vars[6]], r[auxin]        
            if len(vars)==8:
               file, p1, p2, p3, p4, p5, p6, p7, p8, aa = r['files'], r[vars[0]], r[vars[1]], r[vars[2]], r[vars[3]], r[vars[4]], r[vars[5]], r[vars[6]], r[vars[7]], r[auxin]       
            elif len(vars)==9:
               file, p1, p2, p3, p4, p5, p6, p7, p8, p9, aa = r['files'], r[vars[0]], r[vars[1]], r[vars[2]], r[vars[3]], r[vars[4]], r[vars[5]], r[vars[6]], r[vars[7]], r[vars[8]], r[auxin] 
            ...
            if len(images) >= batch_size:
               if len(vars)==1:            
                  ...
                  yield [np.array(a), np.array(images)], [np.array(p1s)] #(np.array(a) -aux_mean) / aux_std
                  images, a, p1s = [], [], []
               elif len(vars)==2:            
                  p1s = np.squeeze(CS[0].transform(np.array(p1s).reshape(-1, 1)))
                  p2s = np.squeeze(CS[1].transform(np.array(p2s).reshape(-1, 1)))  
                  ...
                  p5s = np.squeeze(CS[4].transform(np.array(p5s).reshape(-1, 1)))
                  a = np.squeeze(CSaux[0].transform(np.array(a).reshape(-1, 1)))                                   
                  yield [np.array(a), np.array(images)], [np.array(p1s), np.array(p2s), np.array(p3s), np.array(p4s), np.array(p5s)]
                  images, a, p1s, p2s, p3s, p4s, p5s = [], [], [], [], [], [], []
               elif len(vars)==6:            
                  p1s = np.squeeze(CS[0].transform(np.array(p1s).reshape(-1, 1)))
                  p2s = np.squeeze(CS[1].transform(np.array(p2s).reshape(-1, 1)))  
                  ...
                  p6s = np.squeeze(CS[5].transform(np.array(p6s).reshape(-1, 1)))                 
                  a = np.squeeze(CSaux[0].transform(np.array(a).reshape(-1, 1)))                                   
                  yield [np.array(a), np.array(images)], [np.array(p1s), np.array(p2s), np.array(p3s), np.array(p4s), np.array(p5s), np.array(p6s)]
                  images, a, p1s, p2s, p3s, p4s, p5s, p6s = [], [], [], [], [], [], [], []
               elif len(vars)==7:            
                  p1s = np.squeeze(CS[0].transform(np.array(p1s).reshape(-1, 1)))
                  p2s = np.squeeze(CS[1].transform(np.array(p2s).reshape(-1, 1)))  
                  ...
                  p7s = np.squeeze(CS[6].transform(np.array(p7s).reshape(-1, 1)))
                  a = np.squeeze(CSaux[0].transform(np.array(a).reshape(-1, 1)))                                   
                  yield [np.array(a), np.array(images)], [np.array(p1s), np.array(p2s), np.array(p3s), np.array(p4s), np.array(p5s), np.array(p6s), np.array(p7s)]
                  images, a, p1s, p2s, p3s, p4s, p5s, p6s, p7s = [], [], [], [], [], [], [], [], []
               elif len(vars)==8:            
                  p1s = np.squeeze(CS[0].transform(np.array(p1s).reshape(-1, 1)))
                  p2s = np.squeeze(CS[1].transform(np.array(p2s).reshape(-1, 1)))  
                  ...
                  p8s = np.squeeze(CS[7].transform(np.array(p8s).reshape(-1, 1)))                              
                  a = np.squeeze(CSaux[0].transform(np.array(a).reshape(-1, 1)))                                   
                  yield [np.array(a), np.array(images)], [np.array(p1s), np.array(p2s), np.array(p3s), np.array(p4s), np.array(p5s), np.array(p6s), np.array(p7s), np.array(p8s)]
                  images, a, p1s, p2s, p3s, p4s, p5s, p6s, p7s, p8s = [], [], [], [], [], [], [], [], [], []                             
               elif len(vars)==9:            
                  p1s = np.squeeze(CS[0].transform(np.array(p1s).reshape(-1, 1)))
                  p2s = np.squeeze(CS[1].transform(np.array(p2s).reshape(-1, 1)))  
                  ...
                  a = np.squeeze(CSaux[0].transform(np.array(a).reshape(-1, 1))) 
                  try:                                  
                     yield [np.array(a), np.array(images)], [np.array(p1s), np.array(p2s), np.array(p3s), np.array(p4s), np.array(p5s), np.array(p6s), np.array(p7s), np.array(p8s), np.array(p9s)]
                  except:
                     pass
                  images, a, p1s, p2s, p3s, p4s, p5s, p6s, p7s, p8s, p9s = [], [], [], [], [], [], [], [], [], [], []
        if not for_training:
...
###===================================================
def predict_test_train_cat(train_df, test_df, train_idx, test_idx, var, SM, classes, weights_path, greyscale):
   """
   ...
   ##==============================================
   ## make predictions on training data
   train_gen = get_data_generator_1image(train_df, train_idx, False, len(classes), var, len(train_idx), greyscale)
   ...
   ## make predictions on testing data
   test_gen = get_data_generator_1image(test_df, test_idx, False, len(classes), var, len(test_idx), greyscale)
   x_test, (true)= next(test_gen)

   #P = []; PT = []
   #for model in models:

   pred = SM.predict(x_test, batch_size=1)
